...
<body>
    ...
    <table class="easyui-datagrid" id="userList" title="会员列表" 
	       data-options="singleSelect:false,collapsible:true,pagination:true,url:'<%=basePath%>user/list.do',method:'get',pageSize:5,toolbar:toolbar,pageList:[2,5,10]">
	    <thead>
	        <tr>
...
<div id="userAdd" class="easyui-window" title="新增会员" data-options="modal:true,closed:true,iconCls:'icon-save',href:'<%=basePath%>user/user_add'" style="width:800px;height:600px;padding:10px;">
        The window content.
</div>
...
var toolbar = [{
    ...
    handler:function(){
    	...
    	//发送选中用户id数据到后台
   		$('#userEdit').dialog({    
   		    ...
   		    closed: false,    
   		    cache: false,    
   		    href: '<%=basePath%>user/queryUserById?ids='+ids,    
   		    modal: true   
   		});    
    ...
    handler:function(){
    	...
    	$.messager.confirm('确认','确定删除ID为 '+ids+' 的会员吗？',function(r){
    	    if (r){
            	$.post("<%=basePath%>user/deleteUserByIds",{'ids':ids}, function(){
